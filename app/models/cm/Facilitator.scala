...
object Facilitator {
   ...
   */
  def updateFacilitatorExperience(repos: Repositories): Unit = repos.cm.license.findAll map { licenses =>
    licenses.foreach { license =>
      ...
      calculateNumberOfEvents(license, repos) map { number =>
        ...
        repos.cm.facilitator.updateExperience(facilitator)
      }
    }
  }
  ...
  protected def calculateNumberOfEvents(license: License, repos: Repositories): Future[Int] =
    repos.cm.event.findByFacilitator(license.licenseeId, Some(license.brandId)).map(_.count(_.confirmed))
}
