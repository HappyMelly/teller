...
class EventRepository(app: Application, services: Repositories) extends HasDatabaseConfig[JdbcProfile]
  ...
  def applyFacilitators(events: List[Event]): Future[Unit] = {
    ...
    futureData flatMap { facilitationData =>
      ...
      services.person.collection.addresses(facilitators) map { _ =>
  ...
  def update(view: EventView): EventView = {
    ...
    db.run(actions)
    services.eventInvoice.update(view.invoice)
