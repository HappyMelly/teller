...
class EvaluationRepository(app: Application, repos: models.repository.Repositories) extends HasDatabaseConfig[JdbcProfile]
  with AttendeeTable
  ...
  def add(eval: Evaluation): Future[Evaluation] = {
    ...
    val actions = (for {
      ...
      _ <- repos.cm.rep.event.attendee.updateEvaluationIdQuery(evaluation.attendeeId, evaluation.id)
  ...
  def delete(evaluation: Evaluation): Future[Unit] = {
    val actions = (for {
      _ <- repos.cm.rep.event.attendee.updateEvaluationIdQuery(evaluation.attendeeId, None)
