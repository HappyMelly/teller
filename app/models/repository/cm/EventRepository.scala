...
import models.cm.{Event, EventInvoice, EventView}
...
class EventRepository(app: Application, repos: models.repository.Repositories) extends HasDatabaseConfig[JdbcProfile]
  ...
  def insert(view: EventView): Future[EventView] = {
    ...
    val actions = (for {
      ...
      _ <- DBIO.sequence(view.event.facilitatorIds(repos).distinct.map { facilitatorId ⇒
  ...
  def applyFacilitators(events: List[Event]): Future[Unit] = {
    ...
    futureData flatMap { facilitationData =>
      ...
      repos.person.collection.addresses(facilitators) map { _ =>
  ...
  def update(view: EventView): EventView = {
    ...
    db.run(actions)
    repos.cm.rep.event.invoice.update(view.invoice)
