...
class EvaluationRepository(app: Application, services: Repositories) extends HasDatabaseConfig[JdbcProfile]
  with AttendeeTable
  ...
  def add(eval: Evaluation): Future[Evaluation] = {
    ...
    val actions = (for {
      ...
      _ <- services.attendee.updateEvaluationIdQuery(evaluation.attendeeId, evaluation.id)
  ...
  def delete(evaluation: Evaluation): Future[Unit] = {
    val actions = (for {
      _ <- services.attendee.updateEvaluationIdQuery(evaluation.attendeeId, None)
