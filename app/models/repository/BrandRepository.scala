...
class BrandRepository(app: Application, services: Repositories) extends HasDatabaseConfig[JdbcProfile]
  with BrandTable
  ...
  def delete(brand: Brand): Unit = {
    services.socialProfile.delete(brand.id.get, ProfileType.Brand)
  ...
  def findByUser(user: UserAccount): Future[List[Brand]] = {
    (for {
      l <- services.license.activeLicenses(user.personId)
      b <- services.brand.findByCoordinator(user.personId)
  ...
  def insert(view: BrandProfileView): Future[Brand] = {
    ...
    db.run(query += view.brand).map { b =>
      services.socialProfile.insert(view.profile.copy(objectId = b.identifier))
      val owner = BrandCoordinator(None, b.identifier, view.brand.ownerId, BrandNotifications(true, true, true))
      services.brandCoordinator.save(owner)
  ...
  def update(old: Brand, view: BrandProfileView, picture: Option[String]): Brand = {
    ...
    services.socialProfile.update(view.profile.copy(objectId = old.identifier), ProfileType.Brand)
 ...
 *
  ...
  protected def updateOwnerRecord(brandId: Long, newOwner: Long, oldOwner: Long): Future[Unit] = {
    (for {
      value <- services.brand.isCoordinator(brandId, newOwner) if value
    } yield ()) map { _ =>
