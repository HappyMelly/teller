...
import models.cm.{Evaluation, Event, Facilitator, OldEvaluation}
  ...
  def receive = {
    ...
    case eventId: Long ⇒
      (for {
        ...
        f <- repos.cm.event.facilitatorIds(eventId)
      } yield (e, f)) map { case (event, facilitators) =>
        facilitators.foreach { id =>
          val evaluationQuery = for {
            events <- repos.cm.event.findByFacilitator(id, Some(event.brandId))
            evaluations <- repos.cm.evaluation.findByEventsWithAttendees(events.map(_.id.get))
            oldEvaluations <- if (event.brandId == MGT30_IDENTIFIER)
              repos.cm.evaluation.findOldEvaluations(id)
            else
          ...
          evaluationQuery map { case (evaluations, oldEvaluations) =>
            ...
            repos.cm.facilitator.update(facilitator.copy(personId = id, brandId = event.brandId))
          }
        }
      }
  }

  def initialCalculation() = repos.cm.facilitator.findAll map { data =>
    data.foreach { x =>
      val evaluationQuery = for {
        events <- repos.cm.event.findByFacilitator(x.personId, Some(x.brandId))
        evaluations <- repos.cm.evaluation.findByEventsWithAttendees(events.map(_.id.get))
        oldEvaluations <- if (x.brandId == MGT30_IDENTIFIER)
          repos.cm.evaluation.findOldEvaluations(x.personId)
        else
          Future.successful(List())
      } yield (evaluations.filter(_._3.approved), oldEvaluations)
      evaluationQuery map { case (evaluations, oldEvaluations) =>
        val facilitator = calculateRatings(evaluations, oldEvaluations)
        repos.cm.facilitator.update(facilitator.copy(personId = x.personId, brandId = x.brandId))
      }
    }
  }
