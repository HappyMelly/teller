...
class PeerCreditsConfigurator @Inject() (val repos: Repositories) extends Actor {
  ...
  protected def updateMultipleAccounts(brandId: Long, on: Boolean) = {
    val request = for {
      c <- repos.brand.coordinators(brandId)
      f <- repos.license.licensees(brandId)
  ...
  protected def updateSingleAccount(personId: Long, brandId: Long, on: Boolean) = {
    ...
    f map { account =>
      if (on && account.credits.isEmpty) {
        ...
        val request = for {
          c <- repos.brand.findByCoordinator(personId)
          l <- repos.brand.findByLicense(personId, onlyActive = true)
        } yield (c ++ l).distinct
        request map { brands =>
