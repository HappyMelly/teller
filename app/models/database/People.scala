...
import models.{ Photo, Address, Person, SocialProfile }
...
private[models] object People extends Table[Person]("PERSON") {
  ...
  def * = id.? ~ firstName ~ lastName ~ emailAddress ~ photo ~ addressId ~ bio ~ interests ~
    boardMember ~ stakeholder ~ webSite ~ blog ~
    virtual ~ active ~ created ~ createdBy ~ updated ~ updatedBy <> (
      { p ⇒
        Person(p._1, p._2, p._3, p._4, Photo.parse(p._5), Address.find(p._6), p._7, p._8,
          SocialProfile.find(p._1.getOrElse(0)), p._9, p._10, p._11, p._12, p._13, p._14, p._15, p._16, p._17, p._18)
      },
      { (p: Person) ⇒
        Some((p.id, p.firstName, p.lastName, p.emailAddress, p.photo.url, p.address.id.get, p.bio, p.interests,
          p.boardMember, p.stakeholder, p.webSite,
          p.blog, p.virtual, p.active, p.created, p.createdBy, p.updated, p.updatedBy))
      })

  def forInsert = * returning id

  def forUpdate = firstName ~ lastName ~ emailAddress ~ photo ~ bio ~ interests ~ boardMember ~ stakeholder ~
    webSite ~ blog ~ virtual ~ updated ~ updatedBy
}
