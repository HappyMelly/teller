...
private[models] trait PersonTable extends AddressTable {
  ...
  class People(tag: Tag) extends Table[Person](tag, "PERSON") {
    ...
    def photoId = column[Option[String]]("PHOTO_ID")
    def signatureId = column[Option[String]]("SIGNATURE_ID")
    ...
    type PeopleFields = (Option[Long], String, String, String,
      Option[LocalDate], Option[String], Option[String], Option[String],
      Long, Option[String], Option[String], Option[String],
      Option[String], Boolean, Boolean,
      DateTime, String, DateTime, String)

    def * = (id.?, firstName, lastName, email, birthday, photo, photoId, signatureId, addressId,
      bio, interests, webSite, blog, virtual, active,
      ...
      (p: Person) ⇒
        Some((p.id, p.firstName, p.lastName, p.email, p.birthday, p.photo.url, p.photo.photoId,
          p.signatureId, p.addressId, p.bio, p.interests, p.webSite, p.blog,
          p.virtual, p.active, p.dateStamp.created,
          p.dateStamp.createdBy, p.dateStamp.updated, p.dateStamp.updatedBy)))

    def forUpdate = (firstName, lastName, email, birthday, bio, interests,
      webSite, blog, virtual, active, updated, updatedBy)
  }
