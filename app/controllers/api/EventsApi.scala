  ...
  val eventDetailsWrites = new Writes[(Event, List[Person])] {
    def writes(view: (Event, List[Person])): JsValue = {
      Json.obj(
        ...
        "website" -> view._1.pageUrl(controllers.cm.routes.Events.public(view._1.hashedId).url),
    ...
    implicit val eventWrites = new Writes[Event] {
      def writes(event: Event): JsValue = {
        ...
        Json.obj(
          ...
          "website" -> event.pageUrl(controllers.cm.routes.Events.public(event.hashedId).url),
